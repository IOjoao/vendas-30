...
def carregar():
    if os.path.exists(ARQUIVO):
        with open(ARQUIVO, "r", encoding="utf-8") as f:
            return json.load(f)
    return [
        {"id": 1, "titulo": "E = mc²", "categoria": "Fisica", "texto": "Energia equivale a massa vezes a velocidade da luz ao quadrado. Com m=0,0004 kg e c=3×10⁸ m/s → E = 3,6 × 10¹³ Joules.", "data": datetime.now().strftime("%d/%m/%Y %H:%M")},
        {"id": 2, "titulo": "Redes Neurais", "categoria": "IA", "texto": "Redes neurais artificiais são inspiradas no cérebro humano. Camadas de neurônios artificiais processam informação em paralelo.", "data": datetime.now().strftime("%d/%m/%Y %H:%M")},
        {"id": 3, "titulo": "Plano Acadêmico", "categoria": "Doutorado", "texto": "Objetivo: Passar no vestibular da UNICAMP 2026 → Bacharelado → Mestrado → Doutorado.", "data": datetime.now().strftime("%d/%m/%Y %H:%M")},
    ]
...
with st.sidebar:
    ...
    if api_key: st.success("✅ API Key configurada!")
    else: st.warning("⚠️ Configure sua API Key para usar o Chat IA")
    st.markdown("---")
    ...
    if salvar_btn and titulo and texto:
        nova = {"id": st.session_state.prox_id, "titulo": titulo, "categoria": categoria, "texto": texto, "data": datetime.now().strftime("%d/%m/%Y %H:%M")}
        st.session_state.notas.insert(0, nova)
        st.session_state.prox_id += 1
        salvar(st.session_state.notas)
        st.success("Anotação salva!")
        st.rerun()
    elif salvar_btn: st.warning("Preencha título e conteúdo.")
...
# ── Abas principais ──────────────────────────────────────────────────────────
aba1, aba2, aba3 = st.tabs(["📝 Anotações", "🤖 Chat com Claude IA", "📚 Documentação do Sistema"])

# ════════════════════════════════════════════════════════════════════════════
# ABA 1 — ANOTAÇÕES
# ════════════════════════════════════════════════════════════════════════════
with aba1:
    filtradas = [n for n in st.session_state.notas if (filtro == "Todas" or n["categoria"] == filtro) and (not busca or busca.lower() in n["titulo"].lower() or busca.lower() in n["texto"].lower())]
    if ordem == "Mais antigas": filtradas = list(reversed(filtradas))
    if not filtradas: st.info("Nenhuma anotação encontrada.")
    else:
        for nota in filtradas:
            badge_cls = CORES_BADGE.get(nota["categoria"], "badge-Outros")
            st.markdown(f'<div class="note-card"><span class="badge {badge_cls}">{nota["categoria"]}</span><div class="note-title">{nota["titulo"]}</div><div class="note-text">{nota["texto"]}</div><small style="color:#45475a">{nota["data"]}</small></div>', unsafe_allow_html=True)
            col1, col2, col3, col4 = st.columns([1, 1.5, 1.5, 4])
            if col1.button("🤖 IA", key=f"ia_{nota['id']}"):
                if not api_key: st.warning("Configure sua API Key na sidebar!")
                else:
...
with aba2:
    ...
    if not api_key:
        ...
        if col_s1.button("📚 Resumir anotações"):
            ...
            with st.spinner("Claude pensando..."): resposta = perguntar_claude(api_key, st.session_state.historico_chat, prompt)
            st.session_state.historico_chat.append({"role": "user", "content": prompt})
            st.session_state.historico_chat.append({"role": "assistant", "content": resposta})
            st.rerun()
        ...
        if col_s3.button("🗑️ Limpar chat"):
            ...
            st.rerun()
        st.markdown("---")
        if not st.session_state.historico_chat: st.info("Nenhuma conversa ainda. Faça uma pergunta abaixo!")
        else:
            for msg in st.session_state.historico_chat:
                if msg["role"] == "user": st.markdown(f'<div class="chat-label">Você</div><div class="chat-msg-user">{msg["content"]}</div>', unsafe_allow_html=True)
                else: st.markdown(f'<div class="chat-label">🤖 Claude</div><div class="chat-msg-ai">{msg["content"]}</div>', unsafe_allow_html=True)
        st.markdown("---")
        with st.form("chat_form", clear_on_submit=True):
            pergunta = st.text_input("Sua pergunta", placeholder="Ex: Explique a equação de Schrödinger...")
            enviar = st.form_submit_button("Enviar ↗", use_container_width=True)
        if enviar and pergunta:
            with st.spinner("Claude pensando..."): resposta = perguntar_claude(api_key, st.session_state.historico_chat, pergunta)
            st.session_state.historico_chat.append({"role": "user", "content": pergunta})
            st.session_state.historico_chat.append({"role": "assistant", "content": resposta})
            st.rerun()

# ════════════════════════════════════════════════════════════════════════════
# ABA 3 — DOCUMENTAÇÃO DO SISTEMA
# ════════════════════════════════════════════════════════════════════════════
with aba3:
    st.markdown("## 📚 Documentação Completa do Sistema")
    st.markdown("---")
    
    # Seção 1: Visão Geral
    st.markdown("### 🎯 Visão Geral")
    st.markdown("""
    O **Minhas Anotações + IA** é um aplicativo web desenvolvido com **Streamlit** e **Python** que permite:
    - Criar e organizar anotações por categorias
    - Buscar e filtrar conteúdo
    - Integrar com a IA **Claude** para resumos e explicações
    ...
    with col_tec3:
        st.markdown("**💾 Dados**")
        st.markdown("- JSON local\n- Session State\n- Datetime")
    
    st.markdown("---")
    
    # Seção 2: Funções do Sistema
    st.markdown("### ⚙️ Funções Principais")
    
    st.markdown("""
    1. **Gerenciamento de Anotações**: Criar, editar, excluir e visualizar anotações organizadas por categorias.
    2. **Busca e Filtros**: Pesquisar por palavras-chave, filtrar por categoria e ordenar por data.
    3. **Chat com IA**: Enviar perguntas para a IA Claude e receber respostas em tempo real, mantendo um histórico de conversas.
    4. **Estatísticas**: Acompanhar o número total de anotações, quantas foram criadas hoje, categorias usadas e total de palavras.
    """)